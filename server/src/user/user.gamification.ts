...
import RoadmapProgressModel from "../roadmap-progress/roadmap-progress.model";
import CertificateModel from "../certificate/certificate.model";
import ActivityModel from "../activity/activity.model";
import CommunityMembershipModel from "../community/membership.model";
import PostModel from "../community/post.model";
import CommentModel from "../community/comment.model";

const DAY_MS = 24 * 60 * 60 * 1000;
const LEVEL_XP_STEP = 1000;

const PROBLEM_XP: Record<string, number> = {
  Easy: 120,
  Medium: 260,
  Hard: 420,
};
...
const fromDayKeyToNumber = (dayKey: string) =>
  Math.floor(new Date(`${dayKey}T00:00:00.000Z`).getTime() / DAY_MS);

const getLevelFromXP = (xp: number) => Math.max(1, Math.floor(xp / LEVEL_XP_STEP) + 1);

const getRankFromLevel = (level: number) => {
  if (level >= 10) return "Legend";
  if (level >= 7) return "Elite";
  if (level >= 5) return "Architect";
...
const buildMonthlyXPSeries = (xpEvents: Array<{ date: Date; amount: number }>) => {
  const now = new Date();
...
export const syncUserGamification = async (userId: string | mongoose.Types.ObjectId) => {
  ...
  const [
    ...
    SubmissionModel.find({ userId: normalizedUserId }).sort({ createdAt: 1 }).lean(),
    RoadmapProgressModel.find({ userId: normalizedUserId }).lean(),
    CertificateModel.find({ userId: normalizedUserId }).sort({ issuedAt: 1 }).lean(),
    CommunityMembershipModel.find({ user: normalizedUserId }).sort({ createdAt: 1 }).lean(),
    PostModel.find({ author: normalizedUserId }).sort({ createdAt: 1 }).lean(),
    CommentModel.find({ author: normalizedUserId }).sort({ createdAt: 1 }).lean(),
    ActivityModel.find({ userId: normalizedUserId }).sort({ createdAt: 1 }).lean(),
  ]);
  ...
  const uniqueAcceptedProblemIds = [
    ...new Set(
      submissions
        .filter((submission: any) => submission.verdict === "Accepted" && submission.problemId)
        .map((submission: any) => submission.problemId.toString())
    ),
  ];

  const problems = await ProblemModel.find({ _id: { $in: uniqueAcceptedProblemIds } })
  ...
  const xpEvents: Array<{ date: Date; amount: number }> = [];
  ...
  acceptedByProblem.forEach((submission, problemId) => {
    ...
    const difficultyKey = difficulty.toLowerCase() as keyof typeof solved;
  ...
  progressDocs.forEach((progress: any) => {
    ...
    (progress.subtopics || []).forEach((subtopic: any) => {
      (subtopic.resources || []).forEach((resource: any) => {
        if (!resource.completed || !resource.completedAt) return;
  ...
  const acceptedSubmissions = submissions.filter((submission: any) => submission.verdict === "Accepted").length;
  ...
  const firstSolvedAt = acceptedByProblem.size
    ...
    : null;
  const firstHardSolvedAt =
    [...acceptedByProblem.values()].find((submission: any) => {
      const problem = problemMap.get(submission.problemId.toString());
      return problem?.difficulty === "Hard";
    })?.createdAt || null;
  const firstRoadmapCompleteAt =
    progressDocs.find((progress: any) => Number(progress.overallProgress) >= 100)?.updatedAt || null;
  const firstCertificateAt =
    (certificates[0] as any)?.issuedAt || (certificates[0] as any)?.createdAt || null;
  ...
  const globalRank = (await UserModel.countDocuments({ xp: { $gt: totalXP } })) + 1;
